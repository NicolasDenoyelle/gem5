...
class SystemFactory:
    ...
    def _mem_from_attr_(type = 'SimpleMemory',
        ...
        if size % SystemFactory.page_size:
            ...
            raise ValueError;
        return cls(range = AddrRange(start = start_addr, size = size),
                   **kwargs)

    @staticmethod
    def new(memories = [ 'SimpleMemory' ],
            addr = 0,
            clock='2GHz'):
        ...
        for string in memories:
            ...
            if "=" in string:
                ...
                mem = SystemFactory._mem_from_attr_(cls_name, addr, **attr)
            else:
                mem = SystemFactory._mem_from_attr_(string, addr)
            mem.port = system.membus.master
            mems.append(mem)
            addr = addr + mem.range.size()
