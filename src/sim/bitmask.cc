...
static const long unsigned int BITMASK_NBITS =
    8 * sizeof(long unsigned int);
...
Bitmask::Bitmask(const size_t i, const size_t ii) {
    for (int j = 0; j < Bitmask::NULONGS; j++)
        mask[j] = 0;
    set_range(i, ii);
}
...
Bitmask::Bitmask(const char *str) {
    ...
    if (str == NULL || !strcasecmp(str, "none") ||
        !strcasecmp(str, "zero") ||
...
num_list_mask:
    ...
    while (tok != NULL) {
        if (set(strtoul(tok, NULL, 10)) == -1) {
            free(strd);
            fatal("Invalid bit number %s. MAXBITS is %lu",
                  tok, Bitmask::MAXBITS);
        }
...
int Bitmask::isbitset(const long unsigned int i) const {
    return (i >= Bitmask::MAXBITS)
               ? -1
               : (mask[BITMASK_NTH(i)] &
                  (1UL << BITMASK_ITH(i))) > 0UL;
}
...
int Bitmask::set_range(const long unsigned int i,
                       const long unsigned int ii) {
    if (i == ii)
        return set(i);
    if (i >= Bitmask::MAXBITS || ii >= Bitmask::MAXBITS ||
        i > ii)
        return -1;
...
long Bitmask::last() const {
    ...
    for (n = Bitmask::NULONGS - 1; n >= 0 && mask[n] == 0;
         n--)
        ;
    if (n < 0)
        return -1;
